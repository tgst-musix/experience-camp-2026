...
export default function Home() {
  ...
  return <main className={`${styles.site} ${styles.finalSite} ${motion.ready}`}>
    ...
    <section className={styles.finalCta} id="contact" data-reveal="true"><div><p className={styles.eyebrow}>2026 CHURCH MUSIC EXPERIENCE CAMP</p><p className={styles.finalHandwriting} aria-label="也許，這就是你回應呼召的下一步。"><HandwrittenText text="也許，這就是你回應呼召的下一步。" /></p><h2 className={styles.finalHeadline}>也許你不需要現在就決定未來，<br />但可以先走進來，<span>真實體驗。</span></h2><p className={styles.finalMeta}>限額 40 名｜10 月 1 日截止報名</p><div className={styles.finalActions}><a className={styles.primaryButton} href={registrationUrl} target="_blank" rel="noreferrer">立即報名 ↗</a><a className={styles.ghostButton} href={lineUrl} target="_blank" rel="noreferrer">加入 LINE 詢問 ↗</a></div></div></section>
