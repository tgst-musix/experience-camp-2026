...
export default function Home() {
  ...
  return <main className={`${styles.site} ${styles.finalSite} ${motion.ready}`}>
    ...
    <section className={styles.hero} id="top"><div className={styles.heroImage}><img src={asset("photos/hero-choir.webp")} alt="台灣神學院禮拜堂中的詩班與指揮" /><span className={styles.heroShade} /></div><div className={`${styles.heroCopy} ${styles.revealFromLeft}`} data-reveal="true"><p className={styles.kicker}>2026 · 教會音樂碩士班體驗營</p><h1>從服事負擔，<br />走向<span>裝備與方向</span></h1><p className={styles.heroLead}>兩天走進真實課堂、禮拜與校園生活，體驗音樂、神學與教會服事如何彼此整合，分辨你的下一步。</p><div className={styles.heroActions}><a className={styles.primaryButton} href={registrationUrl} target="_blank" rel="noreferrer">立即報名 <i>↗</i></a><a className={styles.ghostButton} href="#contact">加入 LINE 詢問 ↓</a></div></div><aside className={`${styles.immersiveBadge} ${styles.revealFromRight}`} data-reveal="true" aria-label="兩天沉浸式體驗，探索你的音樂呼召"><span>兩天</span><strong>沉浸式體驗</strong><small>探索你的音樂呼召</small></aside><div className={styles.heroMeta}><span><b>DATE <em>營會日期</em></b><strong>10.12–13</strong></span><span><b>PLACE <em>地點</em></b><strong>台灣神學院</strong></span><span><b>LIMIT <em>名額限制</em></b><strong>40 名</strong></span><span><b>DEADLINE <em>報名截止</em></b><strong>10.01</strong></span></div><p className={styles.artNote}>台灣神學院禮拜堂與詩班實景</p></section>
    ...
    <section className={styles.finalCta} id="contact" data-reveal="true"><div><p className={styles.eyebrow}>2026 CHURCH MUSIC EXPERIENCE CAMP</p><p className={`${styles.finalHandwriting} ${motion.finalSentence}`} aria-label="也許，這就是你回應呼召的下一步。"><HandwrittenText text="也許，這就是你回應呼召的下一步。" /></p><h2 className={styles.finalHeadline}>也許你不需要<br className={motion.mobileBreak} />現在就決定未來，<br />但可以先走進來，<br className={motion.mobileBreak} /><span>真實體驗。</span></h2><p className={styles.finalMeta}>限額 40 名｜10 月 1 日截止報名</p><div className={styles.finalActions}><a className={styles.primaryButton} href={registrationUrl} target="_blank" rel="noreferrer">立即報名 ↗</a><a className={styles.ghostButton} href={lineUrl} target="_blank" rel="noreferrer">加入 LINE 詢問 ↗</a></div></div></section>
